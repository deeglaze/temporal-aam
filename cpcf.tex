...
%
The additional important step is how to handle the uncertainy allowed in the definition of $\matches$ in \autoref{sec:temporal-semantics}: $\partial$'s codomain becomes $\wp(\TContract \times \Valuation)$, where we let $\motcon_\bot = \bot_\must$.
%
...
\begin{figure}
  ...
  $\begin{array}{rl}
    ...
    \wedge,\vee &\text{ glb, lub for } \bot < \may < \must \\
  \end{array}$
  \end{tabular}
