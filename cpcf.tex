...
%
Example temporal properties are, ``a file can only be closed if it has been opened'' and, ``if function A is given a function B, then that B may not be called once A returns.''
%
...
%
The language for temporal contracts is an extension of regular expressions with back-references, as many desirable properties are difficult to express in LTL with back-references (DLTL ~\cite{ianjohnson:jlo})\footnote{A feeling expressed by DLTL's inventor~\citep{boddenadmission}}.
%
...
%
It is well-known that control-flow analysis can be stated in terms of model-checking, but this observation is misleading; one must use quadratically many model-checking queries to discover what a monovariant flow analysis can discover in one run, and significantly more if polyvariant (most spurious and ignored by a control-flow analysis).
%
...
\begin{itemize}
 \item{an alternative to \citeauthor{ianjohnson:dfm:icfp2011}'s temporal contract semantics;}
...
%
This area has similarities to temporal contracts due to their runtime monitoring and languages for describing execution traces, but none have the concept of blame or a static story.
%
Indeed, the name ``verification'' is a misnomer, as runtime monitoring cannot prove the absence of bugs for all executions, a property we attribute to formal verification.
%
The notion of an event is reminiscent of aspect-oriented programming's notion of a \emph{join-point}, and thus we see several systems built on AspectJ that offer a domain-specific language for running \emph{advice} when the event trace matches a specified pattern, e.g. Tracematches and J-LO ~\citep{ianjohnson:Allan05addingtrace, ianjohnson:jlo}.
%
...
%
If the \emph{now} portion of the proposition fails to hold, the monitor raises a failure (not a blame).
%
J-LO is closer to temporal contracts in its goal, but not its language, whereas tracematches match the language and not the goal.
%
...
%
Similar to our approach, Bandera employs flow analysis in order to produce compact models, but unsimilarly, it does not synthesize checkers for runtime monitoring the expressed properties, nor does it natively support higher-order functions.
%%
...
%
Model-checking an untyped HORS is undecidable, and they make various approximations biased towards to soundly model-check programs in traditional type systems rather than traditional untyped languages.
%
...
%
Additionally, the AAM approach makes extensions to more difficult language features straightforward, whereas in HORS one would need to CPS, double CPS, or perform a functional encoding of a new form of data, all of which impose additional proof obligations/points-of-failure for the analysis implementor.
%
Finally, HORS do not synthesize runtime monitors or have a notion of blame.
...
%
The important aspect of the abstraction process is not this transformation, but the fact that the space of values becomes \emph{finite}, meaning the space of temporal contract derivatives is finite (a simple coinduction).
%
Therefore, the only aspects of THOcons that we need to tamper with are allowing binding to sets of values, and the notion of equality, to weaken it to be conservative with our abstractions.
%
...
\end{itemize}
%
Then, any time a temporal contract is in a \emph{may} state, we blame conservatively.
%
...
\end{itemize}
%
We built and evaluated all these but the last.
%%
