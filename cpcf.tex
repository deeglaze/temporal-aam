...
%
  Contract violations during testing help correct both the implementation and specification of software without having to provide a proof of correctness up-front.
%%

%%
  Painful though it is to admit, impure programming constructs are here to stay, and we need better tools to tame them;
%
  temporal contracts are a proposed declarative language for specifying and dynamically monitoring adhereance to stateful protocols.
%
...
%
\citep{ianjohnson:dthf:complete} show that contracts that simulate types will never violate the guarantee that well-typed programs will never be blamed in a gradually-typed system.
%
...
%
Bandera~\citet{ianjohnson:bandera} is a sophisticated tool-chain that uses several static analysis techniques to extract a finite model from a Java program to feed to various back-end model-checkers.
%
...
\begin{mathpar}
  ...
  \inferrule{\pi\mstate \in \paths(\mgraph) \\
