...
%
If $\mname$ is a label ($\mmlab$), we simply check that the monitor wrapping the function has the same label (attached via the structural contract).
%
...
%
Patterns can match constants ($\mconstant$), variables ($\mvar$), or simply anything ($\swc$).
%%

%%
%TODO: Should we describe how they are problematic?
