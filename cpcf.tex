...
\begin{figure}
  \begin{align*}
    \toltl(\mevent, \mgraph, \maccum, \menv) &= \bigwedge\limits_{(\mevent', \mgraph', \menv') \in \splitpattern(\mevent, \mgraph, \menv)}{\ltland{\mevent'}{\ltlnext{\toltl(\maccum, \mgraph', \sddd, \menv')}}}
    \\
    \toltl(\snonevent{\mevent}, \mgraph, \maccum, \menv) &= \bigvee\limits_{A \in \ltlactions \setminus \setof{\mevent}}{A}
    \\
    \toltl(\stnot{\mtcon}, \mgraph, \maccum, \menv) &= \ltlnot{\toltl(\mtcon, \mgraph, \stnot{\maccum}, \menv)} % WRONG
    \\
    \toltl(\stOr{\mtcon_0}{\mtcon_1}, \mgraph, \maccum, \menv) &= \ltlor{\toltl(\mtcon_0, \mgraph, \maccum, \menv)}{\toltl(\mtcon_1, \mgraph, \maccum, \menv)}
    \\
    \toltl(\stseq{\mtcon_0}{\mtcon_1}, \mgraph, \maccum, \menv) &= \toltl(\mtcon_0, \mgraph, \stseq{\mtcon_1}{\maccum}, \menv)
    \\
    \toltl(\stmany{\mtconNoB}, \mgraph, \maccum, \menv) &= \ltlW{\toltl(\mtconNoB, \mgraph, \sddd, \menv)}{\toltl(\maccum, \mgraph, \sddd, \menv)}
    \\
    \toltl(\sddd, \mgraph, \sddd, \menv) &= \ltlTrue
    \\
    \toltl(\sddd, \mgraph, \maccum, \menv) &= \ltlW{\ltlTrue}{\toltl(\maccum, \mgraph, \sddd, \menv)}
  \end{align*}
...
\mtcon \in \TContract &::=
 ...
 \alt \stnot{\mtcon}
 \alt \stOr{\mtcon_1}{\mtcon_2}
 \alt \stseq{\mtcon}{\mtcon}
 \alt \stmany{\mtconNoB}
 \alt \sddd \\
&\text{ where $\mtcon_1$ and $\mtcon_2$ do not cross-bind } \\ %TODO: explain cross-bind in prose
\mtconNoB \in \TContract \backslash \mbevent &\quad\text{ same as $\mtcon$, but without binding } \\
\mevent \in \Event &::= \scallev{\mname}{\mvpat} \alt \sretev{\mname}{\mvpat} \alt \mbevent \\
\mbevent \in \Event &::= \scallev{\mbfun}{\mbvar} \alt \sretev{\mbfun}{\mbvar} \\
