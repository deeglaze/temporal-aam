...
%
The syntax is presented in figure \ref{fig:tcontract-syntax}.
%
\begin{figure}
  \begin{align*}
 \mtcon \in \TContract &::=
...
%
One of their motivating examples (recalled in figure \ref{fig:sort}) was to protect the comparator passed to a sort function from escaping the scope of the call.
%
...
\begin{figure}
 ...
 \begin{flalign*}
  open &: TCPSocket\ \to\ TCPSendContract \\
  listen &: TCPSocket\ \to\ TCPRecvContract
 \end{flalign*}
 $TCPData =$
 ...
 \begin{flalign*}
  ...
  &\quad\send{\tcpfin}~ \rcv{\tcpack}) \\ &\cup
    (\call{close}~ \send{\tcpfin} \\
  &\quad\rcv{\tcpack}~ \rcv{\tcpfin}~ \send{\tcpack})) \\
 ...
 \begin{flalign*}
  ...
  &\rcv{\tcpack}~ \stmany{\notclose} \\
  &((\call{close}~ \send{\tcpfin} \\
  &\quad\rcv{\tcpack}~ \rcv{\tcpfin}~ \send{\tcpack}) \\ &\cup
    (\rcv{\tcpfin}~ \send{\tcpack} \\
  &\quad\call{close}~ \send{\tcpfin}~ \rcv{\tcpack})) \\
  &\ret{timeout}~ \ret{close}~ \sddd \\
  \text{and}\quad
  &\stnot{(\sddd~ \ret{close}~ \sddd~ \call{send})}
 \end{flalign*}
