...
&\alt \stcall{\mvar}{\mname}{\mtcon}
 ...
 \alt \stwrap{\mtoplevelname}{\mname}{\mtcon} \\
...
%
However, if $\mvar$ is a name, then we consult a binding environment that the monitoring system builds as we pass binding events to determine if the function is a wrapped closure around the pointer bound by a wrap event.
%
...
%
The wrapping process for a first-class function is then also an event, since it produces a new value --- the one that actually emits the call and return events.
%
...
\\
   ...
   \Traces^\mtimeline
\\
   \denotetcon{\stcall{\mvar}{\mname'}{\mtcon}}{\mtimeline}{\menv} &=
   \setbuild{\mtrace \cdot \mtrace'}
            {\mtrace \in \denoteevent{\scallev{\mvar}{\mval}}{\mtimeline}{\menv}, 
             \mtrace' \in \denotetcon{\mtcon}{\mtimeline}{\menv[\mname' \mapsto \mval]}}
\\
   \denotetcon{\stret{\mvar}{\mname'}{\mtcon}}{\mtimeline}{\menv} &=
   \setbuild{\mtrace\cdot \mtrace'}
            {\mtrace \in \denoteevent{\sretev{\mvar}{\mval}}{\mtimeline}{\menv},
             \mtrace' \in \denotetcon{\mtcon}{\mtimeline}{\menv[\mname' \mapsto \mval]}}
\\
   \denotetcon{\stwrap{\mtoplevelname}{\mname}{\mtcon}}{\mtimeline}{\menv} &=
   \setbuild{\apwrapev{\mtoplevelname}{\mval} \cdot \mtrace}
            {\mtrace \in \denotetcon{\mtcon}{\mtimeline}{\menv[\mname' \mapsto \mval]}}
  \end{align*}
...
\begin{figure}
  \begin{align*}
    \denoteevent{\scallev{\mname}{\mvpat}}{\mtimeline}{\menv} &= \\
    &\left\{\begin{array}{ll}
           \setof{\apvcall{\menv(\mname)}{\mathit{ppat}(\mvpat, \menv)}} &\text{ if } \timelineis{\menv(\mname)}{\mtimeline} \\
...
\\
   \mathit{ppat}(\mname, \menv) &= \menv(\mname)
\\
...
\\
    \match(\swc, \mdata, \mbindenv) &= \smust{\mbindenv}
\\
    \match(\sref{\mname}, \mdata, \mbindenv) &= \weakif(\mbindenv, \mdata \simeq^\top \mbindenv(\mname))
\\
    \match(\sbind{\mname}, \mdata, \mbindenv) &= \smust{\mbindenv[\mname \mapsto \mdata]}
\\
...
\\
    \epshelp : \TContract \times \wp(\State) \times \Timeline \times \State \times \Nat \times \mathit{Transition} \to\span\omit\\
             \phantom{:} \mathit{Transition} \times \wp(\State) \times \State \times \Nat\span\omit
\end{align*}
and
\begin{align*}
\begin{array}{l}
  \epshelp(\scallev{\mname}{\mvpat'}, F, \mtimeline, \pmsmqcur, k, \delta) = \\
           (\delta
            \cup \left\{\begin{array}{l}
                        (\pmsmqcur,
                         \epsilon,
                         \apvcall{\sref(\mname)}{\mathit{ppat}(\mvpat)},
                         \pmsmq_k),\\
                        (\pmsmqcur,
                         \epsilon,
                         \apvcall{\bclos{\swc}{\swc}{\swc}{\swc}{\mtimeline}{\swc}{\swc}{\sref{\mname}}}{\mathit{ppat}(\mvpat)}{\swc},
                         \pmsmq_k)
                        \end{array}\right\}, \\
  ...
  \epshelp(\scallev{\mtoplevelname}{\mvpat'}, F, \mtimeline, \pmsmqcur, k, \delta) = \\
           (\delta
            \cup \left\{\begin{array}{l}
                        (\pmsmqcur,
                         \epsilon,
                         \apvcall{\bclos{\mtoplevelname}{\swc}{\swc}{\swc}{\mtimeline}{\swc}{\swc}{\swc}}{\mathit{ppat}(\mvpat)}{\mtimeline},
                         \pmsmq_k),\\
                        (\pmsmqcur,
                         \epsilon,
                         \apvcall{\bclos{\mtoplevelname}{\swc}{\swc}{\swc}{\mtimeline}{\swc}{\swc}{\swc}}{\mathit{ppat}(\mvpat)}{\swc},
                         \pmsmq_k)
                        \end{array}\right\}, \\
            \ F\cup \setof{\pmsmq_k}, \pmsmq_k, k+1)
\\
  \epshelp(\stwrap{\mtoplevelname}{\mname}{\mtcon}, F, \mtimeline, \pmsmqcur, k, \delta) = \\
\begin{array}{l}
   (\delta'' \cup \setof{(\pmsmqlast, -, \epsilon, \pmsmq_{k'})}, F' \cup \setof{\pmsmq_{k'}}, \pmsmq_{k'}, k' + 1) \\
   \text{where } \delta' = \delta \cup \setof{(\pmsmqcur, +, \apwrapev{\mtoplevelname}{\sbind{\mname}}{\mtimeline}, \pmsmq_{k+1})} \\
   \phantom{\text{where } } (\delta'', F', \pmsmqlast, k') = \epshelp(\mtcon, F \cup \setof{\pmsmqcur}, \mtimeline, \pmsmq_k, k+1, \delta')
\end{array} \\
\begin{array}{ll}
  \mathit{ppat}(\swc, \mtimeline) &= \swc
\\
  \mathit{ppat}(\mname, \mtimeline) &= \sref{\mname}
\\
...
%
\begin{lemma}
  $\mtrace \in \denotetcon{\mtcon}{\mtimeline}{\bot}$ iff $\mtrace$ accepted by $\translateeps(\mtcon, \mtimeline)$ and $\wf(\mtrace)$.
\end{lemma}
\begin{proof}
  \textbf{TODO (Ian)}
\end{proof}
...
&\alt \sSMon{\mmlab}{\mmlab}{\mlab}{\mscon}{\mexp}
 \alt \sTMon{\mmlab}{\mmlab}{\mlab}{\mcontract}{\mexp} \\
...
\mctx{\mmlab} \in \ECtx &::=
 ...
 \alt \mlctx{\mmlab} \\
&\alt \sSMon*{\ell}{k}{j}{\mtimeline}{\mcontract}{\mctx*}
 \alt \sSMon*{\ell^\prime}{k}{j}{\mtimeline}{\mcontract}{\mctx{\ell}} \\
&\alt \sTMon{\ell}{k}{j}{\mscon}{\mctx*}
...
\mstate^\prime \angle{\mctx{\mmlab^\prime}[\ldots], \msto^\prime, \mTMons^\prime}
\\ \hline % If true
...
\\ % Temporal monitor
\sTMon{k}{\mmlab}{j}{\mscon \times \mtcon}{\mval}
&\machstep&
 \\ % TODO 
...
&\machstep&
\sown{\bclos{\mname}{\mscon_D}{k}{j}{\mtimeline}{\mscon_R}{\mmlab}{\maddr}}{\mmlab} \\
&&\text{where } \maddr = alloc(\mstate) \\
&&\text{given } \mval = \slam{\mvar}{\mexp}
\\ % Arrow monitor fail
\ditto
&\machstep&
\sblame{k}{j} \quad\text{given } \mval \neq \slam{\mvar}{\mexp}
\\ % Check true
...
\sblame{k}{j}
\\ % Blessed application
\sapp{ \sOwn{\bclos{\mname}{\mscon_D}{k}{j}{\mtimeline}{\mscon_R}{\mmlab^{\prime\prime}}{\maddr}}{\mmlab} }{ \sOwn{\mval}{\mmlab} }
&\machstep&
%TODO: Should the timeline be different?
\sret{\mname}{\mtimeline}{ \sapp{\mval_\lambda}{\mval} } \\
%TODO: Do we even need to explicitly deref here?
&&\text{where } \mval_\lambda = \msto(\maddr) \\
&&\phantom{\text{where }} \mTMons^\prime = \sstep{\mTMons}{\mtimeline}{\scallev{\mname}{\mval}} \\
&&\text{given $\mTMons(\mtimeline)$ accepts $\scallev{\mname}{\mval}$}
\\ % Blessed application fail
\ditto
&\machstep&
\sblame{\mmlab}{j} \quad\text{given $\mTMons(\mtimeline)$ does not accept $\scallev{\mname}{\mval}$}
\\ % Return
\sret{\mname}{\mtimeline}{ \sOwn{\mval}{\mmlab} }
&\machstep&
\mval \\
&&\text{where } \mTMons^\prime = \sstep{\mTMons}{\mtimeline}{\sretev{\mname}{\mval}} \\
&&\text{given $\mTMons(\mtimeline)$ accepts $\sretev{\mname}{\mval}$}
\\ % Return fail
\ditto
&\machstep&
\sblame{\mname}{\j}
\quad\text{given $\mTMons(\mtimeline)$ does not accept $\sretev{\mname}{\mval}$}
\\ % Dereference
\maddr &\machstep& \mval \quad\text{where } \mval = \msto(\maddr)
\\ \hline % Blame
