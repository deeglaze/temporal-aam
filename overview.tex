...
&\alt \stOr{\mtcon\ldots}
 ...
 \alt \stbind{\mevent}{\mtcon}
\\
\mevent \in \Event &::= \scevev{\mname}{\mvpat} \alt \sany \\
\mcev \in \FunctionEvent &::= {\tt call} \alt {\tt ret} \\
\mvpat \in \VPat &::= \mconstant \alt \sbind{\mvar} \alt \mname \alt \scons(\mvpat,\mvpat) \alt \snegpat{\mvpat} \alt \sany \alt \snone \\
...
%
Patterns can match constants ($\mconstant$), variable bindings and references ($\sbind{\mvar}$, $\mvar$), labeled functions ($\mtoplevelname$), structured data ($\scons(\mvpat,\mvpat)$), negated patterns that do not bind ($\snegpat{\mvpat}$), anything or nothing ($\sany$, $\snone$).
%%
